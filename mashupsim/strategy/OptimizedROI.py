...
def sigmoid(x) : 
    return 0.5 + 0.5 / ( 1.0 + math.exp(-2*x))

class OptimizedROI (StrategyStateMachine) :


    ## 在服务发现的时候模拟有ROI的指导的时候的情况与表现。具体来说，ROI比率较低的时候，要求更高的质量，说明服务本身的不行。
    ## 但是ROI是一个实数，还需要看如何比较
    ...
    def discover(self):
        ...
        for case in self.val_service_solutions :
            ...
            ## 没有运行的方案，就直接使用原来的
                ...
                if (sigmoid(self.val_ROI) < self.env_w.do_eval_case_quality(self.env_w.selected_case)) : 
                    self.env_w.assign_best_case(self.env_w.selected_case)
                    yield self.goto(self.composite)
                else : 
                    yield self.env.process(self.discover())
