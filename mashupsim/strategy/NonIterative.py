...
class NonIterativeStrategy (StrategyStateMachine) :
    ...
    def discover(self):
        ...
        for case in self.val_service_solutions :
            ...
            if self.val_judging_units : ## 空列表，空集，以及None返回的都是False

                ## 更好的方案，替代原有sf_judging_quality
                #if (self.val_sf_judging_quality < self.env_w.do_eval_case_quality(self.env_w.selected_case)) : 
                #    self.env_w.assign_best_case(self.env_w.selected_case)
                ## 只要评估组件的质量大于某个值，比如0.8，就可以部署结果了，这个时候可以设置更多的组合，比如30个
                if (0.7 < self.env_w.do_eval_case_quality(self.env_w.selected_case)) : 
                    self.env_w.assign_best_case(self.env_w.selected_case)
                else : ## 候选方案还不如当前选择的最优方案，因此丢弃
                    pass
            else : ## 没有方案的时候最优的方案就是当前选择的
                self.env_w.assign_best_case(self.env_w.selected_case)

        else : 
            if self.val_judging_units : ## 发现最优方案
