...
class Workload2 :
    ...
    def do_service_select(self, time=1) :
        ...
        self.selected_case = current

        self.action_cost = self.cost_ref[0]
        self.action_time = self.time_ref[0]
        self._action_name = "generate service select case"
    ...
    def do_eval_case_quality(self, case=[]) :
        ...
        if not case : 
            ...
            self.action_cost = self.cost_ref[0]
            self.action_time = self.time_ref[0]
            self._action_name = "valuing select case quality"
            return CaseEvaluator(self.env_s, self.selected_case).fitness_sf_quality(self.env_u)
        else :
            return self.do_eval_any_case_quality(case)

    ## 评估计算任何方案的花费
    def do_eval_any_case_quality(self, case) :


        self.action_cost = self.cost_ref[0]
        self.action_time = self.time_ref[0]
        self._action_name = "valuing select case quality"
        return CaseEvaluator(self.env_s, case).fitness_sf_quality(self.env_u)

    ## 最终由策略作出决策，哪一个方案是最好的
    def assign_best_case(self, case) :
        self.judging_units = case
    ...
    def undo_unit_consult_if_useless(self, time=1) :
        ...
        # 只有那些老服务中不再需要的才需要协商
        print("RUNNING", self.running_service)
        print("DEV", self.developed_service)
